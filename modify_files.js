...
const appJsPath = 'C:\\Users\\BRD\\Downloads\\PS\\fittrack\\app.js';
const styleCssPath = 'C:\\Users\\BRD\\Downloads\\PS\\fittrack\\style.css';
...
if (!appJs.includes('function vectorSvg')) {
    appJs = appJs.replace("document.addEventListener('DOMContentLoaded', () => {", "document.addEventListener('DOMContentLoaded', () => {\n" + vectorSvgCode);
}

// 2. Update exerciseLibrary
const exerciseLibraryOld = `  const exerciseLibrary = [
...
const exerciseLibraryNew = `  const exerciseLibrary = [
...
appJs = appJs.replace(exerciseLibraryOld, exerciseLibraryNew);

// 3. Update renderExercises
const renderExercisesOld = `      // Mock animation placeholder using CSS
      card.innerHTML = \`
...
const renderExercisesNew = `      card.innerHTML = \`
...
appJs = appJs.replace(renderExercisesOld, renderExercisesNew);

// 4. Update openExerciseDetail
const openExerciseDetailOld = `    document.getElementById('tut-execution').textContent = ex.exec;
    
    const errList = document.getElementById('tut-errors');`;

const openExerciseDetailNew = `    document.getElementById('tut-execution').textContent = ex.exec;
    
    document.getElementById('tut-animation').innerHTML = vectorSvg(ex.svgType || 'bench', ex.color || '#7c3aed');

    const errList = document.getElementById('tut-errors');`;

appJs = appJs.replace(openExerciseDetailOld, openExerciseDetailNew);

// 5. Update Calendar logic
const calendarOld = `  function renderCalendar() {
...
const calendarNew = `  const calendarSchedule={
...
appJs = appJs.replace(calendarOld, calendarNew);
...
styleCss = styleCss.replace(cssCalOld, cssCalNew);
...
`;
if (!styleCss.includes('/* NEW VECTOR ANIMATIONS */')) {
    styleCss += "\n" + vectorAnimCss;
}
